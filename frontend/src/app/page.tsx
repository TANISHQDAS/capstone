...
export default function EasyBeginnerDashboard() {
  ...
  const [tasks, setTasks] = useState([
    {
      ...
      owner: 'Marcus',
      dueDate: '2026-09-12',
      status: 'pending', // 'pending', 'created', 'rejected'
      ticketId: ''
    },
    {
      id: 't-2',
  ...
  return (
    <div className="max-w-[1200px] mx-auto p-4 md:p-8 space-y-6">
      ...
      <header className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-mono">Turn Meeting Audio & Text into Verified Work Tickets</p>
          </div>
        </div>

        {/* Dummy PDF Report Download Button */}
        <div className="flex items-center gap-2">
          <a
            href="http://localhost:8000/download-pdf"
            target="_blank"
            rel="noopener noreferrer"
            className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-xs px-3.5 py-2 rounded-lg border border-indigo-200 shadow-sm transition-all flex items-center gap-1.5"
          >
                  ...
                  <div key={item.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3">
                    ...
                    {item.status === 'created' && (
                      <div className="text-xs font-mono text-emerald-700 font-bold bg-emerald-50 p-2 rounded border border-emerald-200 flex items-center gap-1">
                        ✓ Ticket created and verified in Linear task tool!
                      </div>
                    )}
        ...
        <div className="md:col-span-4 space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-3 border-t-4 border-t-indigo-600">
            <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider font-mono flex items-center justify-between">
              ...
              <span className="text-xs text-indigo-600 font-normal">PDF File</span>
            </h3>

            <p className="text-xs text-slate-600 leading-relaxed">
              Generate and view a dummy PDF report of the fetched meeting transcript, participants, and extracted tasks.
            </p>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs py-2.5 rounded-lg shadow-sm transition-all flex items-center justify-center gap-2 font-mono"
            >
              📄 Open / Download Dummy PDF Report
            </a>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-3">
            ...
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 text-xs font-mono space-y-2 text-slate-700">
              ...
              <ol className="list-decimal list-inside space-y-1.5 text-slate-600">
                ...
                <li>Extracts who needs to do what</li>
                <li>Creates work tickets automatically</li>
              </ol>
            </div>
          </div>
      ...
      <footer className="text-center text-xs text-slate-500 font-mono py-4 border-t border-slate-200 flex flex-wrap justify-between gap-2">
        <div>Meeting Intelligence Agent &bull; Easy to Understand Project</div>
